import {TYEvent} from "./TYEvent";

export class _TYEventDispatcher{
    ...
    dispatchEventWith(type: string, data?: any): boolean {
        if (this.hasEventListener(type)) {
            let event: TYEvent = TYEvent.create(TYEvent, type);
            event.data = data;
    ...
    private notifyListener(event: TYEvent) {
...
export const TYEventDispatcher = new _TYEventDispatcher();
